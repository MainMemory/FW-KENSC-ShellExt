...
public:
	...
	AdjListNode(size_t dest, size_t dist, size_t len, size_t wgt)
		: destnode(dest), weight(wgt), distance(dist), length(len) {
	}
	AdjListNode(AdjListNode const &other) = default;
	//AdjListNode(AdjListNode &&other) = default;
	AdjListNode &operator=(AdjListNode const &other) = default;
	//AdjListNode &operator=(AdjListNode &&other) = default;
	// Getters.
 ...
 * 	// one single symbol..
 * 	constexpr static size_t symbolwise_weight();
